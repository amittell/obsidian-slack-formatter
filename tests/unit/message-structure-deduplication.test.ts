...
import { SlackMessage } from '../../src/models.js';
import { EmbeddedMessageDetector, EmbeddedContentType } from '../../src/formatter/processors/embedded-message-detector.js';
import { ContentDeduplicationProcessor } from '../../src/formatter/processors/content-deduplication-processor.js';
import { MessageStructureValidator, ValidationSeverity } from '../../src/formatter/validators/message-structure-validator.js';
import { OutputFormattingStandards, ContentType } from '../../src/formatter/standards/output-formatting-standards.js';
...
4`;
    ...
    describe('ContentDeduplicationProcessor', () => {
        it('should remove duplicate content blocks', () => {
            const messages = [
                ...
                createMessage('User2', 'Another message with https://example.com/link\nLink Title\nLink description'),
                createMessage('User3', 'Response to the first message'),
                createMessage('User2', 'https://example.com/link\nLink Title\nLink description\nSame link shared again')
            ];
    ...
    describe('MessageStructureValidator', () => {
        ...
        it('should validate timestamp presence and format', () => {
            ...
            const timestampIssues = result.issues.filter(i => 
                i.type === 'missing_timestamp' || i.type === 'invalid_timestamp'
            );
    ...
    describe('Integration Tests', () => {
        it('should process embedded content and deduplicate in sequence', () => {
            const messages = [
                createMessage('User1', `Original message
https://example.com/link
Link Title
Link Description`),
                createMessage('User2', 'Response to original'),
                createMessage('User1', `Same link again:
https://example.com/link
Link Title
Link Description`)
            ];
