...
describe('Username Extraction Comprehensive Test Suite', () => {
    describe('Format Detection Edge Cases', () => {
        it('should detect thread format with complex emoji patterns', () => {
            const complexEmojiLine = "John DoeJohn Doe![:custom-emoji:](https://emoji.slack-edge.com/url)![:another:](url2) [Monday at 4:28 PM](https://stripe.slack.com/archives/p123)";
        ...
        it('should detect thread format with Unicode emoji in username', () => {
            const unicodeEmojiLine = "Jane Smith 🔥Jane Smith 🔥 [Tuesday at 5:30 PM](https://slack.com/archives/p456)";
    ...
    describe('Thread Format Username Extraction', () => {
        it('should extract usernames with various emoji combinations', () => {
            const testCases = [
                {
                    input: "Bill MeiBill Mei![:emoji1:](url1)![:emoji2:](url2) [Monday at 4:28 PM](https://example.com)",
                    expected: "Bill Mei"
                },
                {
                    input: "Single NameSingle Name![:single-emoji:](url) [Tuesday at 5:30 PM](https://example.com)",
                    expected: "Single Name"
                },
                {
                    input: "Three Part NameThree Part Name![:complex:](url) [Wednesday at 6:00 PM](https://example.com)",
    ...
    describe('Full Username Extraction Pipeline', () => {
        it('should handle complete thread format extraction', () => {
            const testCases = [
                {
                    input: "Bill MeiBill Mei![:connect-fingerguns:](https://emoji.slack-edge.com/url) some extra text",
                    format: MessageFormat.THREAD,
    ...
    describe('Username Extraction Metrics', () => {
        it('should provide comprehensive extraction statistics', () => {
            const testData = [
                // Thread format cases
                { input: "Bill MeiBill Mei![:emoji:](url) [12:00 PM](link)", format: MessageFormat.THREAD, expectedUser: "Bill Mei" },
                { input: "User Name [12:01 PM](link)", format: MessageFormat.THREAD, expectedUser: "User Name" },
                { input: "Complex UserComplex User![:e1:](u1)![:e2:](u2) [12:02 PM](link)", format: MessageFormat.THREAD, expectedUser: "Complex User" },
                
                // DM format cases
                { input: "Alex MittellAlex Mittell", format: MessageFormat.DM, expectedUser: "Alex Mittell" },
                { input: "Single User", format: MessageFormat.DM, expectedUser: "Single User" },
                { input: "Test UserTest User :emoji:", format: MessageFormat.DM, expectedUser: "Test User" },
                
                // Edge cases
                { input: "", format: MessageFormat.DM, expectedUser: "Unknown User" },
                { input: "Malformed input", format: MessageFormat.UNKNOWN, expectedUser: "Malformed input" }
            ];
