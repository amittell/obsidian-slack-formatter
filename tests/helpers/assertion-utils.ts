...
export function assertUserValidation(
    ...
    const unexpected = detected.filter(user => 
        !expectedUsers.includes(user) && user !== 'Unknown User'
    );
...
export function assertMessageBoundaries(
    ...
    testCases.forEach((testCase, index) => {
        ...
        if (testCase.contentIncludes) {
            const hasContent = matchingMessages.some(m => 
                m.text?.includes(testCase.contentIncludes!)
            );
...
export function assertUserDifferentiation(
    ...
    differentiationRules.forEach(rule => {
        const matchingMessages = messages.filter(msg => 
            msg.username === rule.username &&
            msg.text?.includes(rule.requiredContent)
        );
...
export function assertNoContentMerging(
    ...
    separationRules.forEach(rule => {
        ...
        const messageWithExpected = userMessages.find(m => 
            m.text?.includes(rule.shouldContain)
        );
...
export function assertRegressionFix(
    ...
    if (regressionChecks.noTimestampAsUsername) {
        const hasTimestampUsername = messages.some(m => 
...
export function createValidationSuite(
    ...
    return {
        ...
        validateContent: () => formattedOutput ? 
            assertContentPreservation(formattedOutput, criticalContent) : null,
        ...
        validateNoRegression: () => assertRegressionFix(messages, {
            noUnknownUser: true,
