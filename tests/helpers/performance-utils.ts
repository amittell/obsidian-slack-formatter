...
export function measurePerformance<T>(
  ...
  const shouldCollectMemory =
    process.env.SHOW_PERFORMANCE === 'true' ||
...
function calculatePerformanceSummary(metrics: PerformanceMetrics[]): PerformanceSummary {
  ...
  const avgMemoryUsage =
    metrics.reduce((sum, m) => sum + (m.memoryUsage?.heapUsed || 0), 0) / metrics.length;
...
export function validatePerformance(
  ...
  if (thresholds.maxAverageTime && benchmark.summary.averageTime > thresholds.maxAverageTime) {
    failures.push(
      `Average time ${benchmark.summary.averageTime.toFixed(1)}ms exceeds threshold ${thresholds.maxAverageTime}ms`
    );
  }

  if (thresholds.maxSingleExecution && benchmark.summary.maxTime > thresholds.maxSingleExecution) {
    failures.push(
      `Max time ${benchmark.summary.maxTime.toFixed(1)}ms exceeds threshold ${thresholds.maxSingleExecution}ms`
    );
  }

  if (
    thresholds.minCharactersPerSecond &&
    benchmark.summary.throughput < thresholds.minCharactersPerSecond
  ) {
    failures.push(
      `Throughput ${benchmark.summary.throughput.toFixed(0)} chars/sec below threshold ${thresholds.minCharactersPerSecond}`
    );
  }
  ...
  if (thresholds.minMessagesPerSecond && benchmark.averageMetrics.messagesPerSecond) {
    if (benchmark.averageMetrics.messagesPerSecond < thresholds.minMessagesPerSecond) {
      failures.push(
        `Message processing rate ${benchmark.averageMetrics.messagesPerSecond.toFixed(1)} msgs/sec below threshold ${thresholds.minMessagesPerSecond}`
      );
    }
  }
  ...
  if (thresholds.maxMemoryUsage && benchmark.averageMetrics.memoryUsage) {
    if (benchmark.averageMetrics.memoryUsage.heapUsed > thresholds.maxMemoryUsage) {
      failures.push(
        `Memory usage ${(benchmark.averageMetrics.memoryUsage.heapUsed / 1024 / 1024).toFixed(1)}MB exceeds threshold ${(thresholds.maxMemoryUsage / 1024 / 1024).toFixed(1)}MB`
      );
    }
  }
 ...
 */
export function logPerformanceBenchmark(
  benchmark: PerformanceBenchmark,
  ...
  if (benchmark.averageMetrics.messagesPerSecond) {
    console.log(
      `Message processing: ${benchmark.averageMetrics.messagesPerSecond.toFixed(1)} messages/second`
    );
  }
  ...
  if (benchmark.averageMetrics.memoryUsage) {
    console.log(
      `Average memory usage: ${(benchmark.averageMetrics.memoryUsage.heapUsed / 1024 / 1024).toFixed(1)}MB`
    );
  }
  ...
  if (showDetails) {
    ...
    benchmark.metrics.forEach((metric, index) => {
      console.log(
        `  ${index + 1}: ${metric.executionTime.toFixed(1)}ms (${metric.charactersPerSecond.toFixed(0)} chars/sec)`
      );
    });
  }
}
