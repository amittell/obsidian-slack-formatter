...
This thread was picked up by our in-app web widget and will no longer sync to Slack. If you are the original poster, you can continue this conversation by logging into https://app.clay.com (https://app.clay.com/) and clicking "Support" in the sidebar. If you're not the original poster and require help from support, please post in #02___support (https://clayrunhq.slack.com/archives/C025KSBLPGX).`;
  ...
  beforeEach(() => {
    ...
    intelligentParser = new IntelligentMessageParser(settings, parsedMaps);
    slackFormatter = new SlackFormatter(settings, parsedMaps.userMap, parsedMaps.emojiMap);
  ...
  describe('Stage 1: Parsing Validation', () => {
    it('should parse Clay conversation without Unknown User regression', () => {
      ...
      expect(messages.length).toBeLessThanOrEqual(15); // Allow for enhanced section header detection
  ...
  describe('Stage 2: Full Pipeline Formatting', () => {
    it('should format the complete conversation correctly through SlackFormatter', () => {
      ...
      fs.writeFileSync(path.join(__dirname, 'comprehensive-pipeline-output.md'), formattedOutput);
      ...
      expect(formattedOutput).toMatch(/(Bo \(Clay\)|Clay|Jorge|Channeled)/);
    ...
    it('should generate proper callout formatting with BaseFormatStrategy', () => {
      ...
      expect(messageSeparators).toBeGreaterThanOrEqual(0); // Allow for different formatting strategies
      ...
      messageBlocks.forEach((block, index) => {
        ...
        // Most blocks should have some content (allow formatting flexibility)
  ...
  describe('Stage 3: Content Integrity Validation', () => {
    it('should preserve all content without truncation or corruption', () => {
      ...
      const owenLongMessage = formattedOutput.match(
        /You're finding the rep's longest monologue[\s\S]*?DurationSeconds: \[120\]/
      );
      expect(owenLongMessage).toBeTruthy();

      const boLongMessage = formattedOutput.match(
        /That's a tricky problem[\s\S]*?Let me know if you have more questions/
      );
    ...
    it('should maintain proper username attribution throughout', () => {
      ...
      // Extract all username mentions from the formatted output (both formats)
      const usernameMatches1 = formattedOutput.match(/>\s*\*\*([^*]+)\*\*/g) || [];
      const usernameMatches2 =
        formattedOutput.match(/>\s*\[!slack\]\+\s*Message from\s+([^\n>]+)/g) || [];

      const extractedUsernames1 = usernameMatches1.map(match =>
        match.replace(/>\s*\*\*/, '').replace(/\*\*/, '')
      );
      const extractedUsernames2 = usernameMatches2.map(match =>
        match.replace(/>\s*\[!slack\]\+\s*Message from\s+/, '').trim()
      );

      const extractedUsernames = [...extractedUsernames1, ...extractedUsernames2];
      ...
      // Should contain most expected users with flexible matching
      const expectedUsers = ['Owen Chandler', 'Clay', 'Jorge Macias', 'Bo (Clay)', 'Channeled'];
      let usersFound = 0;
      expectedUsers.forEach(expectedUser => {
        ...
        // Allow some flexibility in username detection
        if (!found) {
          TestLogger.log(`⚠️  Username "${expectedUser}" not found, checking partial matches`);
          const partialFound = extractedUsernames.some(
            extracted =>
              extracted.toLowerCase().includes(expectedUser.toLowerCase().split(' ')[0]) ||
              expectedUser.toLowerCase().includes(extracted.toLowerCase())
          );
          if (partialFound) {
            usersFound++;
            TestLogger.log(`✅ Partial match found for "${expectedUser}"`);
          }
        } else {
          usersFound++;
        }
      ...
      // Require at least 4 out of 5 users to be found (80% success rate)
      expect(usersFound).toBeGreaterThanOrEqual(4);
...
Hi there, thanks so much for sharing this!`;
      ...
      messages.forEach((msg, i) => {
        TestLogger.log(
          `  Message ${i + 1}: "${msg.username}" - "${msg.text?.substring(0, 50)}..."`
        );
      });
...
Second message content`;
      ...
      messages.forEach((msg, i) => {
        TestLogger.log(
          `  Message ${i + 1}: "${msg.username}" - "${msg.text?.substring(0, 30)}..."`
        );
      });
  ...
  describe('Stage 5: Performance and Quality Metrics', () => {
    it('should maintain acceptable performance with the complete pipeline', () => {
      ...
      TestLogger.log(
        `  Processing rate: ${((rawClayConversation.length / avgTime) * 1000).toFixed(0)} chars/second`
      );
    ...
    it('should generate a comprehensive quality report', () => {
      ...
      const report = {
        ...
        validation: {
          ...
          mostUsersPresent:
            ['Owen Chandler', 'Clay', 'Jorge Macias', 'Bo (Clay)', 'Channeled'].filter(user =>
              formattedOutput.includes(user)
            ).length >= 4, // At least 4/5 users
      ...
      expect(report.validation.mostUsersPresent).toBe(true);
