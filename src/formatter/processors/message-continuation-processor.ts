...
export class MessageContinuationProcessor extends BaseProcessor<SlackMessage[], SlackMessage[]> {
   ...
   */
  private isContinuationCandidate(
    message: SlackMessage,
    lastKnownAuthor: SlackMessage | null
  ): boolean {
    ...
    const isTimestampWithContent =
      timestampPatterns.some(pattern => pattern.test(firstLine)) && lines.length > 1;
  ...
  private mergeContinuation(targetMessage: SlackMessage, continuation: SlackMessage): void {
    ...
    if (continuation.threadInfo && typeof continuation.threadInfo === 'string') {
