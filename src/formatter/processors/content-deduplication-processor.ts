...
export class ContentDeduplicationProcessor {
  ...
  process(messages: SlackMessage[]): DeduplicationResult {
    ...
    if (this.debugMode) {
      result.debugInfo = {
        ...
        preservedBlocks: contentBlocks.filter(
          block => !duplicateBlocks.some(dup => dup.messageIndex === block.messageIndex)
        ),
      };
  ...
  private extractContentBlocks(analysisResults: EmbeddedDetectionResult[]): ContentBlock[] {
    ...
    if (this.debugMode) {
      Logger.debug('ContentDeduplicationProcessor', 'Starting content block extraction', {
        messageCount: analysisResults.length,
      });
    }

    analysisResults.forEach((result, messageIndex) => {
      if (this.debugMode) {
        Logger.debug('ContentDeduplicationProcessor', `Analyzing message ${messageIndex}`, {
          username: result.message.username,
          textLength: result.message.text.length,
          hasEmbedded: result.hasEmbedded,
          embeddedCount: result.embeddedContent.length,
      ...
      if (result.message.text.trim().length >= DEDUPLICATION_CONFIG.MIN_CONTENT_LENGTH) {
        ...
        if (this.debugMode) {
          Logger.debug(
            ...
            `Added main content block for message ${messageIndex}`,
            {
              username: result.message.username,
              content: mainBlock.content,
              length: mainBlock.content.length,
            }
          );
        }
      } else {
        if (this.debugMode) {
          Logger.debug(
            'ContentDeduplicationProcessor',
            `Skipped main content block for message ${messageIndex} (too short)`,
            {
              username: result.message.username,
              textLength: result.message.text.trim().length,
              minRequired: DEDUPLICATION_CONFIG.MIN_CONTENT_LENGTH,
            }
          );
        }
      }
      ...
      result.embeddedContent.forEach((embedded, embeddedIndex) => {
        ...
        if (embeddedText.length >= DEDUPLICATION_CONFIG.MIN_CONTENT_LENGTH) {
          ...
          if (this.debugMode) {
            Logger.debug(
              'ContentDeduplicationProcessor',
              `Added embedded content block ${embeddedIndex} for message ${messageIndex}`,
              {
                username: result.message.username,
                type: embedded.type,
                content: embeddedText,
          ...
          if (this.debugMode) {
            Logger.debug(
              'ContentDeduplicationProcessor',
              `Skipped embedded content block ${embeddedIndex} for message ${messageIndex} (too short)`,
              {
                username: result.message.username,
                type: embedded.type,
                length: embeddedText.length,
    ...
    if (this.debugMode) {
      Logger.debug('ContentDeduplicationProcessor', 'Content block extraction complete', {
        totalBlocks: blocks.length,
        mainBlocks: blocks.filter(b => !b.isEmbedded).length,
        embeddedBlocks: blocks.filter(b => b.isEmbedded).length,
      });
    }
  ...
  private identifyDuplicates(contentBlocks: ContentBlock[]): ContentBlock[] {
    ...
    if (this.debugMode) {
      Logger.debug('ContentDeduplicationProcessor', 'Starting duplicate identification', {
        totalBlocks: contentBlocks.length,
        threshold: DEDUPLICATION_CONFIG.SIMILARITY_THRESHOLD,
        maxDistance: DEDUPLICATION_CONFIG.MAX_MESSAGE_DISTANCE,
      });

      contentBlocks.forEach((block, index) => {
        Logger.debug('ContentDeduplicationProcessor', `Block ${index}`, {
          messageIndex: block.messageIndex,
          isEmbedded: block.isEmbedded,
          embeddedType: block.embeddedType,
    ...
    for (let i = 0; i < contentBlocks.length; i++) {
      ...
      if (this.debugMode) {
        Logger.debug(
          'ContentDeduplicationProcessor',
          `Comparing block ${i} against all subsequent blocks`,
          {
            currentBlock: {
              messageIndex: currentBlock.messageIndex,
              isEmbedded: currentBlock.isEmbedded,
              contentPreview: currentBlock.content.substring(0, 30) + '...',
            },
          }
        );
      }
      ...
      for (let j = i + 1; j < contentBlocks.length; j++) {
        ...
        const isExact = this.isExactMatch(currentBlock.content, compareBlock.content);

        if (messageDistance > DEDUPLICATION_CONFIG.MAX_MESSAGE_DISTANCE && !isExact) {
          if (this.debugMode) {
            Logger.debug(
              'ContentDeduplicationProcessor',
              `Skipping comparison (distance too large)`,
              {
                blockI: i,
                blockJ: j,
                messageDistance,
                maxAllowed: DEDUPLICATION_CONFIG.MAX_MESSAGE_DISTANCE,
                isExact,
              }
            );
          }
        ...
        if (this.debugMode) {
          Logger.debug('ContentDeduplicationProcessor', `Similarity calculation`, {
            blockI: i,
            blockJ: j,
            similarity: similarity.toFixed(3),
            threshold: DEDUPLICATION_CONFIG.SIMILARITY_THRESHOLD,
            isDuplicate: similarity >= DEDUPLICATION_CONFIG.SIMILARITY_THRESHOLD,
            currentContent: currentBlock.content.substring(0, 30) + '...',
            compareContent: compareBlock.content.substring(0, 30) + '...',
          });
        }
        ...
        if (similarity >= DEDUPLICATION_CONFIG.SIMILARITY_THRESHOLD) {
          ...
          if (this.debugMode) {
            Logger.debug(
              'ContentDeduplicationProcessor',
              `Found duplicate - selecting block to remove`,
              {
                blockI: i,
                blockJ: j,
                similarity: similarity.toFixed(3),
                currentBlock: {
                  messageIndex: currentBlock.messageIndex,
                  isEmbedded: currentBlock.isEmbedded,
                  embeddedType: currentBlock.embeddedType,
                },
                compareBlock: {
                  messageIndex: compareBlock.messageIndex,
                  isEmbedded: compareBlock.isEmbedded,
                  embeddedType: compareBlock.embeddedType,
                },
                blockToRemove: blockToRemove === currentBlock ? 'current (i)' : 'compare (j)',
              }
            );
          }
          ...
          if (blockToRemove === compareBlock) {
            ...
            if (this.debugMode) {
              Logger.debug('ContentDeduplicationProcessor', `Marked block ${j} for removal`, {
                messageIndex: compareBlock.messageIndex,
                content: compareBlock.content.substring(0, 50) + '...',
              });
            }
          } else {
            ...
            if (this.debugMode) {
              Logger.debug('ContentDeduplicationProcessor', `Marked block ${i} for removal`, {
                messageIndex: currentBlock.messageIndex,
                content: currentBlock.content.substring(0, 50) + '...',
              });
            }
    ...
    if (this.debugMode) {
      Logger.debug('ContentDeduplicationProcessor', 'Duplicate identification complete', {
        totalDuplicates: duplicates.length,
        duplicatesByMessage: duplicates.reduce(
          (acc, dup) => {
            acc[dup.messageIndex] = (acc[dup.messageIndex] || 0) + 1;
            return acc;
          },
  ...
  private calculateCharacterSimilarity(str1: string, str2: string): number {
    // Use safe whitespace removal to prevent ReDoS attacks
    const set1 = new Set(str1.toLowerCase().replace(/[ \t\r\n]/g, ''));
    const set2 = new Set(str2.toLowerCase().replace(/[ \t\r\n]/g, ''));
  ...
  private calculateEditSimilarity(str1: string, str2: string): number {
    ...
    const maxAllowedDistance = Math.floor(maxLength * 0.3); // 70% similarity minimum
    if (lengthDiff > maxAllowedDistance) {
      return 0.0; // Too different in length
    }

    const distance = this.optimizedLevenshteinDistance(str1, str2, maxAllowedDistance);
    if (distance === -1) return 0.0; // Exceeded threshold
   ...
   */
  /**
   * Optimized Levenshtein distance with early termination and space efficiency.
   *
   * Uses rolling arrays (O(min(m,n)) space instead of O(m*n)) and early termination
   * when distance exceeds threshold. This provides significant performance improvement
   * for similarity calculations where we only care about distances below a threshold.
   *
   * @param str1 First string for distance calculation
   * @param str2 Second string for distance calculation
   * @param maxDistance Maximum allowed distance before early termination
   * @returns Distance between strings, or -1 if exceeds maxDistance
   * @complexity O(min(m,n)*max(m,n)) time, O(min(m,n)) space with early termination
   */
  private optimizedLevenshteinDistance(str1: string, str2: string, maxDistance: number): number {
    ...
    // Use rolling arrays instead of full matrix (space optimization)
    ...
    for (let j = 1; j <= n; j++) {
      ...
      // Early termination: if minimum distance in row exceeds threshold, abort
  ...
  /**
   * Legacy Levenshtein distance (kept for compatibility/fallback)
   * @deprecated Use optimizedLevenshteinDistance for better performance
   */
  ...
  private getEmbeddedTypePriority(type?: EmbeddedContentType): number {
    ...
    switch (type) {
      case EmbeddedContentType.QUOTED_MESSAGE:
        return 4;
      case EmbeddedContentType.FILE_ATTACHMENT:
        return 3;
      case EmbeddedContentType.LINK_PREVIEW:
        return 2;
      case EmbeddedContentType.METADATA:
        return 1;
      case EmbeddedContentType.REACTIONS:
        return 0;
      case EmbeddedContentType.CONTINUATION:
        return 0;
      default:
        return 0;
    }
  ...
  private applyDeduplication(
    ...
    if (this.debugMode) {
      Logger.debug('ContentDeduplicationProcessor', 'Starting deduplication application', {
        messageCount: analysisResults.length,
    ...
    analysisResults.forEach((result, messageIndex) => {
      ...
      if (this.debugMode) {
        Logger.debug('ContentDeduplicationProcessor', `Processing message ${messageIndex}`, {
          username: result.message.username,
      ...
      const messageDuplicates = duplicateBlocks.filter(
        block => block.messageIndex === messageIndex
      );

      if (this.debugMode) {
        Logger.debug(
          'ContentDeduplicationProcessor',
          `Found ${messageDuplicates.length} duplicates for message ${messageIndex}`,
          {
            username: result.message.username,
            duplicates: messageDuplicates.map(d => ({
              isEmbedded: d.isEmbedded,
              embeddedType: d.embeddedType,
              contentPreview: d.content.substring(0, 30) + '...',
            })),
          }
        );
      }
      ...
      if (messageDuplicates.length > 0) {
        // Check if the main message content is marked as duplicate
        const mainContentIsDuplicate = messageDuplicates.some(block => !block.isEmbedded);

        if (this.debugMode) {
          Logger.debug(
            'ContentDeduplicationProcessor',
            `Duplicate analysis for message ${messageIndex}`,
            {
              username: result.message.username,
              mainContentIsDuplicate,
              embeddedDuplicateCount: messageDuplicates.filter(d => d.isEmbedded).length,
              mainDuplicateCount: messageDuplicates.filter(d => !d.isEmbedded).length,
            }
          );
        }

        if (mainContentIsDuplicate) {
          if (this.debugMode) {
            Logger.debug(
              'ContentDeduplicationProcessor',
              `SKIPPING ENTIRE MESSAGE ${messageIndex} (main content is duplicate)`,
              {
                username: result.message.username,
                originalText: result.message.text,
              }
          ...
          // The main message content is a duplicate - skip this entire message
          return;
        } else {
          // Only embedded content is duplicate, clean it but keep the main message
          if (result.hasEmbedded && result.cleanedText.trim()) {
            ...
            if (this.debugMode) {
              Logger.debug(
                'ContentDeduplicationProcessor',
                `Using cleaned text for message ${messageIndex}`,
                {
                  username: result.message.username,
                  originalText: result.message.text,
                  cleanedText: result.cleanedText,
            ...
            // Clean specific duplicate content
            const cleanedText = this.cleanMessageContent(message.text, messageDuplicates);
            message.text = cleanedText;

            if (this.debugMode) {
              Logger.debug(
                'ContentDeduplicationProcessor',
                `Applied specific content cleaning for message ${messageIndex}`,
                {
                  username: result.message.username,
                  originalText: result.message.text,
      ...
      if (message.text.trim().length > 0) {
        ...
        if (this.debugMode) {
          Logger.debug('ContentDeduplicationProcessor', `Added message ${messageIndex} to output`, {
            username: result.message.username,
        ...
        if (this.debugMode) {
          Logger.debug(
            'ContentDeduplicationProcessor',
            `SKIPPED message ${messageIndex} (no meaningful content remaining)`,
            {
              username: result.message.username,
    ...
    if (this.debugMode) {
      Logger.debug('ContentDeduplicationProcessor', 'Deduplication application complete', {
        originalMessageCount: analysisResults.length,
        processedMessageCount: processedMessages.length,
        removedMessageCount: analysisResults.length - processedMessages.length,
      });
    }
  ...
  private removeEmbeddedContent(text: string, embeddedContent: string): string {
    ...
    // Clean up extra whitespace with safe regex (prevent ReDoS)
