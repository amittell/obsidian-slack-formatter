...
const EMBEDDED_PATTERNS = {
  ...
  /** File attachments with metadata blocks - only match standalone filenames or markdown links */
  FILE_ATTACHMENT: /^\[(.*)\]\((.*)\)$|^([^\s]+\.(pdf|doc|docx|zip|png|jpg|jpeg|gif))$/i,

  /** Quoted/embedded messages from link previews - made more specific to avoid matching regular content */
  EMBEDDED_MESSAGE: /^([A-Za-z\s]+\s+\[[0-9:]+\s*(AM|PM)?\])$/,

  /** Embedded metadata lines (titles, descriptions) */
  METADATA_LINE: /^[A-Z][a-z\s]*[a-z]$|^(Google Doc|PDF|Zip)$/,

  /** Reaction continuation lines */
  REACTION_CONTINUATION: /^:[^:]+:\s*\d+$|^[\d\s]+$/,
} as const;
...
export class EmbeddedMessageDetector {
  ...
  analyzeMessage(message: SlackMessage): EmbeddedDetectionResult {
    ...
    if (this.debugMode && embeddedContent.length > 0) {
      Logger.debug(
        ...
        `Detected ${embeddedContent.length} embedded content blocks`,
        {
  ...
  private detectReactionContinuation(lines: string[], startIndex: number): EmbeddedContent | null {
    ...
    if (
      content.filter(
        line => line.trim() && EMBEDDED_PATTERNS.REACTION_CONTINUATION.test(line.trim())
      ).length > 0
    ) {
      return {
        type: EmbeddedContentType.REACTIONS,
  ...
  private looksLikeMetadata(line: string): boolean {
    return EMBEDDED_PATTERNS.METADATA_LINE.test(line) || (line.length < 60 && /^[A-Z]/.test(line));
  }
  ...
  private looksLikeDescription(line: string): boolean {
    return line.length > 20 && line.length < 200 && !line.includes('[') && !line.includes('http');
  }
  ...
  private looksLikeFileLink(line: string): boolean {
    return (
      EMBEDDED_PATTERNS.FILE_ATTACHMENT.test(line) ||
  ...
  private looksLikeFileMetadata(line: string): boolean {
    return /^(PDF|Doc|Zip|Google Doc)$/i.test(line) || /^\d+ files?$/i.test(line);
  }
  ...
  private looksLikeQuotedMessageStart(line: string): boolean {
    return (
      EMBEDDED_PATTERNS.EMBEDDED_MESSAGE.test(line) &&
      line.length < 100 &&
      (!line.includes(':') || /\[[0-9:]+\s*(AM|PM)?\]/.test(line)) &&
      /^[A-Z]/.test(line)
    );
  }
  ...
  private looksLikeQuotedMessageContent(line: string): boolean {
    return line.length < 200 && !line.includes('http') && !this.looksLikeNewMessage(line);
  }
  ...
  private looksLikeNewMessage(line: string): boolean {
    return /^\d{1,2}:\d{2}/.test(line) || /^[A-Z][a-z]+ [A-Z][a-z]+/.test(line);
  }
