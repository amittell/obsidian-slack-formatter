...
export class FormatStrategyFactory implements IFormatStrategyFactory {
  ...
  public registerStrategy(type: FormatStrategyType, constructor: FormatStrategyConstructor): void {
    if (this.strategyConstructors.has(type)) {
      Logger.warn(
        ...
        `Strategy type "${type}" is already registered. Overwriting.`
      );
    }
  ...
  public updateDependencies(settings: SlackFormatSettings, parsedMaps: ParsedMaps): void {
    Logger.info(
      'FormatStrategyFactory',
      'Updating dependencies and clearing cached strategy instances.'
    );
  ...
  public getStrategyByType(type: FormatStrategyType): FormatStrategy | null {
    ...
    if (!constructor) {
      Logger.warn(
        'FormatStrategyFactory',
        `No strategy constructor registered for type "${type}".`
      );
      return null;
    }
    ...
    if (!this.currentSettings || !this.currentParsedMaps) {
      Logger.error(
        'FormatStrategyFactory',
        `Cannot instantiate strategy "${type}" because dependencies (settings/maps) have not been set.`
      );
      return null;
    }
    ...
    try {
      ...
      Logger.error('FormatStrategyFactory', `Error instantiating strategy for type "${type}".`, {
        error,
      });
      return null;
    }
  ...
  public clearStrategies(): void {
    Logger.info(
      'FormatStrategyFactory',
      'Clearing all registered constructors and cached instances.'
    );
