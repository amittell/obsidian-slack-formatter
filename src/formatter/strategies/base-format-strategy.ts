...
export abstract class BaseFormatStrategy implements FormatStrategy {
  ...
  constructor(settings: SlackFormatSettings, parsedMaps: ParsedMaps) {
    ...
    this.codeBlockProcessor = new CodeBlockProcessor({
      enableCodeBlocks: settings.detectCodeBlocks,
      isDebugEnabled: isDebug,
    });
    this.urlProcessor = new UrlProcessor({ isDebugEnabled: isDebug });
    this.threadLinkProcessor = new ThreadLinkProcessor({
      enableThreadLinks: settings.highlightThreads,
      isDebugEnabled: isDebug,
    });
    // Pass the parsed map and debug flag directly
    this.emojiProcessor = new EmojiProcessor({
      emojiMap: this.parsedMaps.emojiMap,
      isDebugEnabled: isDebug,
    });
  ...
  formatToMarkdown(messages: SlackMessage[]): string {
    ...
    for (const message of messages) {
      ...
      if (!message.text || /^\s*$/.test(message.text)) {
        this.log('debug', `Skipping message object with no text content`, {
  ...
  protected formatThreadInfo(threadInfo: string): string[] {
    ...
    const lastReplyMatch =
      threadInfo.match(/Last reply\s+(.+?)(?:View thread|$)/i) ||
      threadInfo.match(/(\d+\s+(?:days?|months?|hours?|minutes?)\s+ago)View thread/i);
    const viewThreadMatch = threadInfo.match(/View thread/i);
    const threadReplyMatch = threadInfo.match(
      /replied to a thread:\s*(?:"([^"]+)"|(.+?)(?=\s*(?:View thread|Last reply|$)))?/i
    );
    ...
    try {
      ...
      if (typeof message.timestamp === 'string') {
        ...
        try {
          ...
          timestampToFormat = linkedTimestampMatch?.[1]
            ? linkedTimestampMatch[1]
            : message.timestamp;
        } catch (regexError) {
        ...
        if (parsedDate && !isNaN(parsedDate.getTime())) {
          ...
          this.log(
            'debug',
            `Could not parse timestamp, returning cleaned original: ${timestampToFormat}`
          );
      ...
      if (error instanceof RegexError) {
        ...
        this.log(
          'error',
          `Unexpected error formatting timestamp: ${error instanceof Error ? error.message : 'Unknown error'}`,
          {
            timestamp: message.timestamp,
