...
export class ImprovedFormatDetector {
  ...
  private scoreContent(content: string): FormatScore {
    ...
    if (matches.dm > 0 || matches.channel > 0) {
      ...
      const hasAvatarImages =
        /!\[\]\(https:\/\/ca\.slack-edge\.com\/E[A-Z0-9]+-U[A-Z0-9]+-[a-f0-9]+-48\)/.test(content);
      ...
      const hasSpecificDoubledPattern =
        /^([A-Za-z\s\u00C0-\u017F]+)\1\s+\[\d{1,2}:\d{2}\s*(?:AM|PM)?\]\(https:\/\/.*\/archives\/[CD][A-Z0-9]+\/p\d+\)/m.test(
          content
        );
      const hasAvatarPlusDoubledName =
        /!\[\]\(https:\/\/ca\.slack-edge\.com\/[^)]+\)\s*\n\s*[A-Za-z\s]+[A-Za-z\s]+\s+\[/.test(
          content
        );
      const dmIndicators = [
        hasAvatarImages,
        hasDoubledUsernames,
        hasUserMentionLinks,
        hasSpecificDoubledPattern,
        hasAvatarPlusDoubledName,
      ].filter(Boolean).length;

      // Check if channel match is primarily just the C-archive URL
      const channelOnlyFromUrl =
        content.includes('/archives/C') &&
        !(
      ...
      const hasStrongChannelIndicators =
        /joined the channel|set the channel|pinned a message to this channel|shared a file:.*to this channel|^#[a-z0-9-]+/m.test(
          content
        );
      const hasChannelArchiveUrl = /\/archives\/C[A-Z0-9]+\//.test(content);
      const hasChannelFormatPatterns =
        /^---\s+[A-Za-z]/m.test(content) ||
        /\[@\w+\]\([^)]+\)(?:\s+\[@\w+\]\([^)]+\)){2,}/.test(content);
      ...
      if (hasMultiPersonDMLayout && !hasStrongChannelIndicators && !hasChannelFormatPatterns) {
        ...
        dmIndicators >= 2 &&
        !hasStrongChannelIndicators &&
        !hasChannelFormatPatterns &&
        channelOnlyFromUrl
      ) {
  ...
  private matchPatternCategory(
    trimmed: string,
    category: keyof typeof this.compiledPatterns,
