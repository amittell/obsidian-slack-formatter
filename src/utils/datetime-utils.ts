...
const MONTH_DAY_REGEX = new RegExp(
  `(?:(${Object.keys(MONTHS).join('|')})[\\s.]*)(\\d{1,2})(?:st|nd|rd|th)?`,
  'i'
);
...
export function parseDate(dateStr: string): Date | null {
  ...
  if (
    ...
    Logger.warn('datetime-utils', 'Failed to extract valid date components', {
      dateStr: cleanedDateStr,
    });
    return null;
  }
  ...
  if (
    ...
    Logger.warn('datetime-utils', 'Invalid date detected after construction (possible rollover)', {
      dateStr: cleanedDateStr,
      year,
      monthIndex,
      day,
  ...
  try {
    ...
    if (timeZone) {
      try {
        ...
        Logger.warn(
          'datetime-utils',
          `Invalid timeZone provided: "${timeZone}". Falling back to local time.`,
          tzError
        );
    ...
    Logger.error('datetime-utils', 'Error formatting date', {
      error,
...
function _parseTimePart(timeStr: string): { hours: number; minutes: number } | null {
  ...
  if (timeMatch && timeMatch[1] && timeMatch[2] && timeMatch[3]) {
    ...
    if (isNaN(hours) || hours < 1 || hours > 12 || isNaN(minutes) || minutes < 0 || minutes > 59) {
      Logger.warn('datetime-utils', 'Invalid time components found after regex match', {
        hours,
        minutes,
        timeStr,
      });
 ...
 */
function _determineBaseDateAndTimeStr(
  ts: string,
  contextDate?: Date | null
): { baseDate: Date; timeStr: string } | null {
  ...
  if (dateMatch && dateMatch[1]) {
    ...
    if (parsedDate) {
      ...
      Logger.warn('datetime-utils', 'Failed to parse explicit date part. Invalid date.', {
        potentialDatePart,
        year,
      });
...
export function parseSlackTimestamp(ts: string, contextDate?: Date | null): Date | null {
  ...
  try {
    ...
    const dayOfWeekMatch = cleaned.match(
    ...
    if (
      timeWithSecondsMatch &&
      timeWithSecondsMatch[1] &&
      timeWithSecondsMatch[2] &&
      timeWithSecondsMatch[3]
    ) {
      const baseDate = contextDate || today;
