...
function extractEmojiCodepointFromUrl(url: string): string | null {
  ...
  const match = url.match(
    /production-standard-emoji-assets\/[\d.]+\/[^/]+\/([a-f0-9-]+)(?:@\d+x)?\.png/
  );
...
export function replaceEmoji(text: string, emojiMap: Record<string, string>): string {
  ...
  result = result.replace(
    /production\s+standard\s+emoji\s+assets/gi,
    'production-standard-emoji-assets'
  );
  ...
  result = result.replace(
    /!\[\]\(https:\/\/[ae]\.(slack-edge\.com|emoji\.slack-edge\.com)[^)]+\)/g,
    match => {
...
export function removeAllEmoji(text: string): string {
  ...
  cleaned = cleaned.replace(
    /[\u{1F300}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F000}-\u{1F02F}\u{1F0A0}-\u{1F0FF}\u{1F100}-\u{1F64F}\u{1F680}-\u{1F6FF}\u{1F900}-\u{1F9FF}\u{2300}-\u{23FF}\u{2B00}-\u{2BFF}\u{3000}-\u{303F}]/gu,
    ''
  );
...
export function containsEmoji(text: string): boolean {
  ...
  if (/!\[\]\(https:\/\/[ae]\.(slack-edge\.com|emoji\.slack-edge\.com)[^)]+\)/.test(text))
    return true;
...
export function extractEmojiCodes(text: string): string[] {
  ...
  const brokenImageMatches = text.match(
    /!\[\]\(https:\/\/emoji\.slack-edge\.com\/[^/]+\/([a-zA-Z0-9_+-]+)\/[^)]+\)/g
  );
