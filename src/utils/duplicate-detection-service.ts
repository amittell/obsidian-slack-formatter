...
export class DuplicateDetectionService {
    ...
    public removeDuplicateBlocks(content: string, messageStartPatterns: RegExp[]): string {
        ...
        if (validPatterns.length === 0) {
            Logger.warn('DuplicateDetectionService', 'No valid regex patterns provided for duplicate block detection, returning original content');
            return content;
        }
    ...
    private createMessageKey(msg: SlackMessage): string {
        ...
        const timestampKey = msg.timestamp ? msg.timestamp.substring(0, this.timestampLength) : 'no-timestamp';
    ...
    private validateRegexPatterns(patterns: RegExp[]): RegExp[] {
        ...
        for (let i = 0; i < patterns.length; i++) {
            ...
            try {
                ...
                if (!pattern || typeof pattern.test !== 'function') {
                    Logger.warn('DuplicateDetectionService', `Invalid regex pattern at index ${i}: not a RegExp object`);
                    continue;
                }
        ...
        Logger.debug('DuplicateDetectionService', `Validated ${validPatterns.length} out of ${patterns.length} regex patterns`);
