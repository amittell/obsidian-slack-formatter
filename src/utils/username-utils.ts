...
export const APP_MESSAGE_PATTERNS = {
  /** Pattern for app messages with URL prefix - captures only app name before timestamps or other text */
  URL_PREFIX:
    /^\s*\(https?:\/\/[^)]+\)([A-Za-z][A-Za-z0-9\-_.]*(?:\s+App)?)\s*(?:\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)|\s+\w.*|$)/,
  /** Pattern for app messages without parentheses - captures only app name before timestamps or other text */
  NO_PARENS:
    /^\s*https?:\/\/[^\s]+\s+([A-Za-z][A-Za-z0-9\-_.]*(?:\s+App)?)\s*(?:\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)|\s+\w.*|$)/,
...
export function cleanupDoubledUsernames(text: string, format?: MessageFormat): string {
  try {
    ...
    const nameChars =
      "[a-zA-Z0-9\\u00C0-\\u017F\\u0100-\\u024F\\u1E00-\\u1EFF\\u0400-\\u04FF\\u4E00-\\u9FFF\\u3040-\\u309F\\u30A0-\\u30FF\\u0590-\\u05FF\\u0600-\\u06FF'-]";
    ...
    const spacedDuplicatePattern = new RegExp(
      `(\\b${nameChars}+(?:\\s+${nameChars}+)*)\\s+\\1\\b`,
      'g'
    );
    ...
    const separatedPartsPattern = new RegExp(
      `(\\b${nameChars}+)\\s+\\1(\\b${nameChars}+)\\s+\\2\\b`,
      'g'
    );
...
export function extractUsernameFromDMFormat(line: string): string {
  try {
    ...
    const multiPersonDMPattern =
      /^([A-Za-z\s\u00C0-\u017F]+)\1\s+\[\d{1,2}:\d{2}\s*(?:AM|PM)?\]\(https:\/\/.*\/archives\/[CD][A-Z0-9]+\/p\d+\)/;
...
export function isAppMessage(text: string): boolean {
  return (
    APP_MESSAGE_PATTERNS.URL_PREFIX.test(text) ||
...
export function extractAppUsername(text: string): string {
  try {
    // Handle multiline case by converting to single line for processing
    const singleLineText = text.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();

    // Pattern 1: (https://app.slack.com/services/...)AppName
    let match = singleLineText.match(APP_MESSAGE_PATTERNS.URL_PREFIX);
    if (match && match[1]) {
      return match[1].trim();
    }

    // Pattern 2: https://app.slack.com/services/... AppName (no parentheses)
    match = singleLineText.match(APP_MESSAGE_PATTERNS.NO_PARENS);
    if (match && match[1]) {
      return match[1].trim();
    }

    // Pattern 3: Handle doubled app names like "GitHubGitHub APP"
    match = singleLineText.match(APP_MESSAGE_PATTERNS.DOUBLED_APP);
    if (match && match[1]) {
      return match[1].trim();
    }
...
function removeEmojisFromText(text: string): string {
  try {
    ...
    cleaned = cleaned
      .replace(
        /[\u{1F300}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F100}-\u{1F1FF}\u{1F200}-\u{1F2FF}]/gu,
        ''
      )
      .trim();
...
export function removeAllDecorations(text: string): string {
  try {
    ...
    cleaned = cleaned.replace(
      /[^a-zA-Z0-9\u00C0-\u017F\u0100-\u024F\u1E00-\u1EFF\u0400-\u04FF\u4E00-\u9FFF\u3040-\u309F\u30A0-\u30FF\u0590-\u05FF\u0600-\u06FF\s\-_.']/g,
      ' '
    );
...
export function normalizeUsername(username: string): string {
  try {
    ...
    normalized = normalized.replace(
      /^[^a-zA-Z0-9\u00C0-\u017F\u0100-\u024F\u1E00-\u1EFF\u0400-\u04FF\u4E00-\u9FFF\u3040-\u309F\u30A0-\u30FF\u0590-\u05FF\u0600-\u06FF]+|[^a-zA-Z0-9\u00C0-\u017F\u0100-\u024F\u1E00-\u1EFF\u0400-\u04FF\u4E00-\u9FFF\u3040-\u309F\u30A0-\u30FF\u0590-\u05FF\u0600-\u06FF\s']+$/g,
      ''
    );
...
export function isValidUsername(username: string): boolean {
  try {
    ...
    // Check length
    if (
      normalized.length < USERNAME_CONFIG.MIN_LENGTH ||
      normalized.length > USERNAME_CONFIG.MAX_LENGTH
    ) {
      return false;
    }
    ...
    if (
      /^[^a-zA-Z\u00C0-\u017F\u0100-\u024F\u1E00-\u1EFF\u0400-\u04FF\u4E00-\u9FFF\u3040-\u309F\u30A0-\u30FF\u0590-\u05FF\u0600-\u06FF]+$/.test(
        normalized
      )
    ) {
      return false;
