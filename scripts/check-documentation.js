...
function analyzeFile(filePath) {
    ...
    lines.forEach((line, index) => {
        ...
        const functionMatch = trimmed.match(/^\s*(export\s+)?(public\s+|private\s+|protected\s+)?(static\s+)?(async\s+)?(\w+)\s*\(/);
        if (functionMatch && !trimmed.includes('=') && !trimmed.includes('=>')) {
            const functionName = functionMatch[5];
            
            // Skip constructors and some common patterns
            if (functionName === 'constructor' || functionName === 'if' || functionName === 'for' || functionName === 'while') {
                return;
            }
            ...
            if (hasJSDoc(lines, index)) {
                ...
                stats.issues.push(`${relativePath}:${index + 1} - Function '${functionName}' missing JSDoc`);
            }
        }
...
function main() {
    ...
    try {
        ...
        const functionCoverage = stats.totalFunctions > 0 
            ? (stats.documentedFunctions / stats.totalFunctions * 100).toFixed(1)
            : 100;
        
        const classCoverage = stats.totalClasses > 0 
            ? (stats.documentedClasses / stats.totalClasses * 100).toFixed(1)
            : 100;
        
        const interfaceCoverage = stats.totalInterfaces > 0 
        ...
        console.log('=====================================');
        console.log(`Functions: ${stats.documentedFunctions}/${stats.totalFunctions} (${functionCoverage}%)`);
        console.log(`Classes: ${stats.documentedClasses}/${stats.totalClasses} (${classCoverage}%)`);
        console.log(`Interfaces: ${stats.documentedInterfaces}/${stats.totalInterfaces} (${interfaceCoverage}%)`);
